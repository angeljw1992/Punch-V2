...
public function fetchAttendanceData($request, $pdfData = false)
{
    ...
    if ($request->filled('from')) {
        ...
        $query->whereDate('BusinessDate', '>=', now()->subDays(5)->toDateString());
    }
    ...
    if ($request->filled('from')) {
        ...
        $absenseData = $absenseData->whereDate('date', '>=', now()->subDays(7)->toDateString());
    }
    if ($request->filled('to')) {
...
public function downloadPdf(Request $request)
{
    ...
    // Obtener empleados sin marcaciones ni excusas
    $unmarkedEmployees = $this->fetchUnmarkedEmployees($from, $to);

    // Renderizar el HTML para la asistencia actual, incluyendo las fechas y los empleados sin marcaciones ni excusas
    ...
    Mail::to('oficinamarcaciones@grupoapextech.com')->send(new AttendancePdfEmail($pdfContent, $userName));
...
{
    // Obtener IDs de empleados que tienen al menos una marcación en la fecha específica
    $employeesWithAttendance = POSTLD_CLCK::whereDate('BusinessDate', $date)
                                          ->groupBy('EmployeeID')
                                          ->havingRaw('COUNT(*) >= 1') // Si tiene 1 o más marcaciones
                                          ->pluck('EmployeeID')
                                          ->toArray();

    // Obtener IDs de empleados que tienen una excusa en la fecha específica
    $employeesWithExcuses = EmployeeAbsenseExcuse::whereDate('date', $date)
                                                 ->pluck('employee_id')
                                                 ->toArray();

    // Excluir estos empleados de la lista de empleados activos
    $excludedEmployeeIds = array_merge($employeesWithAttendance, $employeesWithExcuses);

    // Obtener los empleados activos que no están en la lista de excluidos
...
public function fetchUnmarkedEmployees($from, $to)
{
    $dates = [];
    ...
    while ($currentDate <= $endDate) {
        ...
        // Obtener los IDs de empleados que tienen al menos una marcación en la fecha actual
        $employeesWithAttendance = POSTLD_CLCK::whereDate('BusinessDate', $dateString)
                                              ->groupBy('EmployeeID')
                                              ->havingRaw('COUNT(*) > 0') // Excluir empleados con 1 o más marcaciones
                                              ->pluck('EmployeeID')
                                              ->toArray();

        // Obtener los IDs de empleados que tienen excusas en la fecha actual
        $employeesWithExcuses = EmployeeAbsenseExcuse::whereDate('date', $dateString)
                                                     ->pluck('employee_id')
                                                     ->toArray();
        ...
        // Obtener los empleados activos que no están en la lista de excluidos
        $unmarkedEmployees = EmployeeList::where('status', 65)
                                         ->whereNotIn('EmployeeID', $excludedEmployeeIds)
                                         ->where('HiringDate', '<=', $dateString) // Validar fecha de contratación
                                         ->select('EmployeeID', 'FullName', 'HiringDate')
                                         ->get()
                                         ->map(function($employee) use ($dateString) {
                                             if ($employee->HiringDate <= $dateString) {
                                                 return [
                                                     'date' => $dateString,
                                                     'id' => $employee->EmployeeID,
                                                     'name' => $employee->FullName
        ...
        if (!$unmarkedEmployees->isEmpty()) {
            $dates[$dateString] = $unmarkedEmployees;
        }
